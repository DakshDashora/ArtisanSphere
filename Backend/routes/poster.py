...
# Initialize Firebase (Cloud Run will use the service account)
if not firebase_admin._apps:
    firebase_admin.initialize_app(
        options={"storageBucket": "artistsphere-cdab2.firebasestorage.app"}
    )
...
async def create_poster(request: PosterRequest, bucket=Depends(get_storage_bucket)):
    ...
    # Construct prompt for image editing
    prompt = f"""
    ...
    - Creative text in beautiful font is encouraged
    - The image should be similar to the provided image
    - Language for any text on the poster: {request.language}.
    """

    # Generate poster (sync in threadpool)
    def edit_image_sync():
    ...
    # Upload to Firebase Storage (sync in threadpool)
